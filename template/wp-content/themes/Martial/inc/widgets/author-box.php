 ...
 */
class Martial_Author_Box_Widget extends WP_Widget
{

	public function __construct()
	{
		parent::__construct(
			'martial-author-box-widget',
			__( 'Author Box Widget', 'martial' ),
			array(
				'description' => __( 'Author box widget for the sidebar', 'martial' ),
			)
		);
	}
	...
	public function widget( $args, $instance )
	{
		...
		echo '<span><img src="' . esc_url( ( $instance['image_url-' . $authorId] ) ? $instance['image_url-' . $authorId] : get_template_directory_uri() . '/images/author_profile.png' ) . '"  width="371" height="267" class="authorphoto" alt="author"></span>';
		echo $args['before_title'] . esc_html( $instance['title-' . $authorId] ) . $args['after_title'];
		echo '<p>' . strip_tags( $instance['textbox-' . $authorId], '<a><span><i><em><strong><b><ul><ol><li><br>' ) . '</p>';
	...
	{
		echo '<div class="martial-author-box-widget-container">';
		echo '<label for="author">' . __( 'Settings for author: ', 'martial' ) . '</label>';
		...
		foreach ( $users as $user ):
			...
			$title = ( isset( $instance['title-' . $user->ID] ) ) ? $instance['title-' . $user->ID] : __( 'AUTHOR PROFILE', 'martial' );
			$textbox = ( isset( $instance['textbox-' . $user->ID] ) ) ? $instance['textbox-' . $user->ID] : __( 'About the author text', 'martial' );
			...
			?>
			<div id="martial-author-box-settings-<?php echo $user->ID; ?>" class="hidden"
				 style="<?php if ( $showUserId === $user->ID ): ?>display: block;<?php endif; ?>">

				<p>
					<label
						for="<?php echo $this->get_field_id( 'title-' . $user->ID ); ?>"><?php _e( 'Title', 'martial' ); ?></label>
				...
				<p>
					<label
						for="<?php echo $this->get_field_id( 'textbox-' . $user->ID ); ?>"><?php _e( 'About:', 'martial' ); ?></label>
				...
				<p>
					<label
						for="<?php echo $this->get_field_id( 'social_twitter-' . $user->ID ); ?>"><?php _e( 'Twitter url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_facebook-' . $user->ID ); ?>"><?php _e( 'Facebook url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_dribbble-' . $user->ID ); ?>"><?php _e( 'Dribbble url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_pinterest-' . $user->ID ); ?>"><?php _e( 'Pinterest url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_linkedin-' . $user->ID ); ?>"><?php _e( 'Linkedin url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_drupal-' . $user->ID ); ?>"><?php _e( 'Drupal url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_wordpress-' . $user->ID ); ?>"><?php _e( 'Wordpress url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_y-combinator-' . $user->ID ); ?>"><?php _e( 'Y-combinator url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_google-plus-' . $user->ID ); ?>"><?php _e( 'Google+ url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_behance-' . $user->ID ); ?>"><?php _e( 'Behance url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_500px-' . $user->ID ); ?>"><?php _e( '500px url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_flickr-' . $user->ID ); ?>"><?php _e( 'Flickr url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_reddit-' . $user->ID ); ?>"><?php _e( 'Reddit url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_youtube-' . $user->ID ); ?>"><?php _e( 'Youtube url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_soundcloud-' . $user->ID ); ?>"><?php _e( 'Soundcloud url', 'martial' ); ?></label>
					...
					<label
						for="<?php echo $this->get_field_id( 'social_medium-' . $user->ID ); ?>"><?php _e( 'Medium url', 'martial' ); ?></label>
				...
				<p>
					<label
						for="<?php echo $this->get_field_id( 'image_url-' . $user->ID ); ?>"><?php _e( 'Image:', 'martial' ); ?></label><br/>
					...
					<input type="button" class="martial-select-img"
						   value="<?php _e( 'Select Image', 'martial' ); ?>"/>
				</p>
			</div>
		<?php
		endforeach;
		echo '<input type="hidden" id="martial-current-author" name="' . $this->get_field_name( 'author' ) . '" value="' . $users[0]->ID . '">';
		echo '</div>';
	}
...
// init the widget
function martial_register_author_box_widget()
{
	register_widget( 'Martial_Author_Box_Widget' );
}

add_action( 'widgets_init', 'martial_register_author_box_widget' );

// queue up the necessary js
function martial_author_box_widget_script_enqueue()
{
	wp_enqueue_style( 'thickbox' );
	wp_enqueue_script( 'media-upload' );
	wp_enqueue_script( 'thickbox' );
	wp_enqueue_script( 'martial_author_box_widget_js', get_template_directory_uri() . '/inc/js/widget-author-box.js', null, null, true );
}

add_action( 'admin_enqueue_scripts', 'martial_author_box_widget_script_enqueue' );